...
-- 연봉은 급여*12 + 급여*커미션*12다. 연봉은 소수점아래를 버린다.
...
FROM (SELECT E.DEPARTMENT_ID, D.DEPARTMENT_NAME, COUNT(*) CNT
      FROM EMPLOYEES E, DEPARTMENTS D
...
SELECT DENSE_RANK() OVER (ORDER BY SALARY ASC) SALARY_RANK,
...
SELECT X.EMPLOYEE_ID, X.FIRST_NAME, (SELECT COUNT(*) 
                                     FROM EMPLOYEES Y
                                     WHERE Y.EMPLOYEE_ID = X.EMPLOYEE_ID) CNT
FROM EMPLOYEES X
